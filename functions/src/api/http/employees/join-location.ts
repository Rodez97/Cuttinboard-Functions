...
export default https.onCall(
  async (
    { locationId, join }: { locationId: string; join?: boolean },
    context
  ) => {
    const { auth } = context;
    ...
    if (join) {
      ...
      // Remove the document from the location employees collection
      batch.delete(
        firestore()
          .collection("Locations")
          .doc(locationId)
      ...
      // Remove the employee from the members array in the location document
      batch.update(
        firestore().collection("Locations").doc(locationId),
        "members",
