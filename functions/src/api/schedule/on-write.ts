...
export default onDocumentWritten("/schedule/{scheduleId}", async (event) => {
  const { data } = event;
  const afterData = data?.after.data() as IScheduleDoc | null | undefined;
  const beforeData = data?.before.data() as IScheduleDoc | null | undefined;
